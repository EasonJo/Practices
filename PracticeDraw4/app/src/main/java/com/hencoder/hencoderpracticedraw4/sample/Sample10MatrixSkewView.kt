...
class Sample10MatrixSkewView : View {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.save()
        matrix.reset()
        matrix.postSkew(0f, 0.5f, (point1.x + bitmapWidth / 2).toFloat(), (point1.y + bitmapHeight / 2).toFloat())
        ...
        canvas.save()
        matrix.reset()
        matrix.postSkew(-0.5f, 0f, (point2.x + bitmapWidth / 2).toFloat(), (point2.y + bitmapHeight / 2).toFloat())
        canvas.concat(matrix)
